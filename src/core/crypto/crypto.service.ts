...
import { EnvType } from '../validators/env';
...
export class CryptoService {
	...
	/**
	 * Creates a deterministic hash for lookup-only tokens.
	 * @param plainText - The text to hash
	 * @returns Hex encoded HMAC-SHA256 hash
	 */
	hash(plainText: string): string {
		return createHmac('sha256', this.key).update(plainText, 'utf8').digest('hex');
	}
}
